...
PATTERN='(pass(word|wd)?|pwd|secret|token|access_?key|api_?key|private_?key|credential)[[:space:]]*[=:][[:space:]]*[^[:alnum:][:space:]]?[A-Za-z0-9#!@%^&*_.+-]{8,}'
...
if [ "$MODE" = "--all" ]; then
  ...
  # 只看暂存区【新增行】
  CANDIDATES="$(git diff --cached -U0 2>/dev/null | grep '^+' | grep -v '^+++' | sed 's/^+//' | grep -Ei "$PATTERN" || true)"
fi
...
if [ -n "$HITS" ] && [ -f "$BASELINE" ]; then
  HITS="$(printf '%s' "$HITS" | grep -vFf "$BASELINE" || true)"
fi
