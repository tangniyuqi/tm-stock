  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" @keyup.enter="onSubmit">
        <el-form-item label="关键词" prop="name">
          <el-input v-model="searchInfo.name" placeholder="请输入关键词" />
        </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="id" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="left" label="关键词" prop="name" min-width="320" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateNewsKeywordFunc(scope.row)">编辑</el-button>
...
import { createNewsKeyword, deleteNewsKeyword, deleteNewsKeywordByIds, updateNewsKeyword, findNewsKeyword, getNewsKeywordList } from '@/api/quant/newsKeyword';
...
const getTableData = async () => {
  const table = await getNewsKeywordList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value });
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteNewsKeywordByIds({ ids });
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功',
...
// 更新行
const updateNewsKeywordFunc = async row => {
  const res = await findNewsKeyword({ id: row.id });
  type.value = 'update';
  if (res.code === 0) {
...
// 删除行
const deleteNewsKeywordFunc = async row => {
  const res = await deleteNewsKeyword({ id: row.id });
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功',
