  ...
  <div class="quant-dashboard">
    <el-header class="header">
      <div class="logo">
        ...
        <span>
          智子量化 - 多任务自动交易系统
          <span class="version" @click="router.push('/layout/quant/update')">
            {{ isPyWebView ? clientVersion : '下载' }}
          </span>
        </span>
      </div>
    ...
    <div class="main-content">
      <Draggable v-model="layoutColumns" tag="el-row" :component-data="{ gutter: 20 }" item-key="id" :animation="200" handle=".column-drag-handle" :force-fallback="true">
        <template #item="{ element }">
          <el-col :span="element.span">
            <div v-if="element.id === 'main'">
              <div class="column-drag-handle" title="可左右拖拽交换位置">
              ...
              <Draggable v-model="dashboardComponents" item-key="id" handle=".card-header, .terminal-header" :animation="200" ghost-class="ghost-card" drag-class="drag-card" :force-fallback="true">
                <template #item="{ element }">
                  <div class="draggable-item">
                    <el-card v-if="element.id === 'account'" class="box-card mb-20">
                      <template #header>
                        <div class="card-header" style="cursor: move">
                          <span class="title">
                            <el-icon><CreditCard /></el-icon>
                            证券账户
                            <span class="account-info" v-if="accountData.account_no">
                            ...
                            <el-tag v-else type="danger" round class="ml-2">未绑定证券账户</el-tag>
                          </span>
                      ...
                      <el-row :gutter="20" class="account-metrics">
                        ...
                        <el-col :span="4">
                          <div class="metric-item">
                            <div class="value position">{{ showFunds ? formatSmartNumber(accountData.market_value) : '****' }}</div>
                            <div class="label">持仓市值 (元)</div>
                          </div>
                        </el-col>
                        <el-col :span="4">
                          <div class="metric-item">
                            <div class="value available">{{ showFunds ? formatSmartNumber(accountData.available_balance) : '****' }}</div>
                            <div class="label">可用资金 (元)</div>
                          </div>
                        </el-col>
                        <el-col :span="4">
                          <div class="metric-item">
                            <div class="value available">{{ showFunds ? formatSmartNumber(accountData.withdrawable_cash) : '****' }}</div>
                            <div class="label">可取资金 (元)</div>
                          </div>
                        </el-col>
                        <el-col :span="4">
                          <div class="metric-item">
                            <div :class="['value', accountData.total_pl_amount >= 0 ? 'text-red' : 'text-green']">
                        ...
                        <el-col :span="4">
                          <div class="metric-item">
                            <div :class="['value', accountData.daily_pl_amount >= 0 ? 'text-red' : 'text-green']">
                    ...
                    <el-card v-if="element.id === 'task'" class="box-card mb-20" :body-style="{ padding: '10px 20px' }">
                      <template #header>
                        <div class="card-header" style="cursor: move">
                          <span class="title">
                            <el-icon><Grid /></el-icon>
                      ...
                      <el-table :data="tradeTasks" stripe style="width: 100%" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="stock" label="任务标的" min-width="100" max-width="150">
                          <template #default="scope">
                            <div class="row-item">
                              <div class="name">{{ scope.row.stock.name }}</div>
                              <div class="symbol">{{ scope.row.stock.ts_code }}</div>
                            </div>
                          </template>
                        </el-table-column>
                      ...
                      <div class="gva-pagination">
                        <el-pagination size="small" layout="total, sizes, prev, pager, next, jumper" :current-page="taskPage" :page-size="taskPageSize" :page-sizes="[5, 10, 20, 30, 50, 100]" :total="taskTotal" @current-change="handleTaskPageChange" @size-change="handleTaskSizeChange" />
                      </div>
                    </el-card>

                    <el-card v-if="element.id === 'record'" class="box-card mb-20" :body-style="{ padding: '10px 20px' }">
                      <template #header>
                        <div class="card-header" style="cursor: move">
                          <span class="title">
                      ...
                      <div class="gva-pagination">
                        <el-pagination size="small" layout="total, sizes, prev, pager, next, jumper" :current-page="recordPage" :page-sizes="[5, 10, 20, 30, 50, 100]" :total="recordTotal" @current-change="handleTradeRecordPageChange" @size-change="handleTradeRecordSizeChange" />
                      </div>
                    </el-card>
              ...
              <el-card class="box-card">
                ...
                <el-tabs v-model="activeInfoTab" type="border-card">
                  <el-tab-pane label="7x24 财经快报" name="news">
                    ...
                    <div class="news-list" @dblclick="pushTo('news')">
                      <div v-if="newsItems.length > 0">
                        <div class="news-item" v-for="(item, index) in newsItems" :key="index">
                          <span>{{ formatToDateTime(item.ctime, 'HH:mm:ss') }} - </span>
                          <span v-html="highlightKeywords(useFormatText(item.content), newsKeyword)"></span>
                  ...
                  <el-tab-pane label="最近60天大事件" name="event">
                    ...
                    <div class="event-list" @dblclick="pushTo('event')">
                      <div v-for="(item, index) in eventItems" :key="index" class="event-item" v-if="eventItems.length > 0">
              ...
              <el-col :span="12">
                <el-form-item label="策略类型" prop="strategy_id" class="form-item">
                  <el-select v-model="taskForm.strategy_id" placeholder="选择策略类型" :loading="strategySearchLoading" remote-show-suffix clearable filterable remote :remote-method="remoteStrategySearch" @change="handleStrategyChange" class="form-control">
                    <el-option v-for="strategy in availableStrategies" :key="strategy.id" :label="strategy.name" :value="Number(strategy.id)" />
                  </el-select>
                </el-form-item>
              </el-col>

              <el-col :span="12">
                <el-form-item label="交易标的" prop="stock.ts_code" class="form-item">
                  <el-select v-model="taskForm.stock.ts_code" placeholder="输入股票名称/代码/拼音首字母" :loading="stockSearchLoading" remote-show-suffix clearable filterable remote :remote-method="remoteStockSearch" @change="handleSymbolChange" class="form-control">
                    <el-option v-for="stock in availableStocks" :key="stock.ts_code" :label="`${stock.name} (${stock.ts_code}) (${stock.market})`" :value="stock.ts_code">
                      <span>{{ stock.name }}</span>
                      <span style="margin-left: 4px; color: var(--el-text-color-secondary); font-size: 13px">({{ stock.ts_code }}) ({{ stock.market }})</span>
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
...
import { Sunny, Moon, InfoFilled, TrendCharts, Operation as SOperation, Monitor, More, Notification, Calendar, Money, CreditCard, Grid, Platform, Delete, CopyDocument, Lightning, Plus, Refresh, RefreshLeft, RefreshRight, Setting, View, Hide, Coin, Search, ChatRound } from '@element-plus/icons-vue';
...
const maskedAccountNo = computed(() => {
  ...
  const start = accNo.substring(0, accNo.length - 8);
  return `${start}****${accNo.slice(-4)}`;
});
...
const taskRules = reactive({
  ...
  'stock.ts_code': [{ required: true, message: '请选择交易标的', trigger: 'change' }],
...
// 处理股票选择变更
const handleSymbolChange = selectedTsCode => {
  const selectedStock = availableStocks.value.find(s => s.ts_code === selectedTsCode);
  if (!selectedStock) return;

  // 检查交易标的市场是否支持
  if (currentAccount.value && currentAccount.value.markets && Array.isArray(currentAccount.value.markets)) {
    if (!currentAccount.value.markets.includes(selectedStock.market)) {
      ElMessageBox.alert(`当前证券账户未开通【${selectedStock.market}】交易权限，可能无法交易此标的；如已开通，请在证券账户设置交易板块。`, '重要提示', {
  ...
  // 将选择的股票完整信息赋值给 taskForm.stock
  Object.assign(taskForm.stock, selectedStock);
  // 自动填充任务名称
  if (!taskForm.name) taskForm.name = selectedStock.name + '-任务01';
};
...
const consoleLogs = ref([
  { time: new Date().toLocaleTimeString(), level: 'INFO', module: 'System', message: '智子量化实盘交易引擎 v1.0.0 已启动。' },
...
onMounted(() => {
  ...
  updateTime();
  timer1 = setInterval(updateTime, 5000);
  checkTradingTime();
  getNews();
  getEvents();
  getTradeTasks();
