  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" tooltip-effect="dark" :data="tableData" row-key="id" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="left" label="基本信息" prop="sender" min-width="150" max-width="240">
          <template #default="scope">
            <div class="cell-style">
              ...
              <el-text tag="p">{{ scope.row.msg_id }}</el-text>
              <el-text tag="p">{{ formatDate(scope.row.created_at) }}</el-text>
            </div>
          </template>
        </el-table-column>

        <el-table-column align="left" label="消息内容" prop="content" min-width="400" max-width="600">
          <template #default="scope">
            ...
            <el-text tag="div" class="text text-content" v-html="highlightKeywords(replaceKeywords(scope.row.content), searchInfo.content)" @mouseup.stop="handleMouseUp" />
        ...
        <el-table-column align="center" label="IC型号" prop="ics" min-width="240" max-width="360">
          <template #default="scope">
            <div class="text text-ics" v-if="scope.row.ics && scope.row.ics.length > 0">
              <el-text class="text-ics-title">找到【{{ scope.row.ics.length }}】个型号</el-text>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="100px">
          <template #default="scope">
            <el-button v-auth="btnAuth.edit" type="primary" link icon="flag" @click="updateMsgFunc(scope.row)">备注设置</el-button>
            <br />
            <el-button type="primary" link icon="user" @click="pushTo('member', scope.row)">创建客户</el-button>
            <br />
            <el-button type="primary" link icon="list" @click="pushTo('follow', scope.row)">创建跟进</el-button>
            <br />
    ...
    >
      <div class="popover-actions">
        <el-button size="small" @click="copySelectedText">复制</el-button>
        <el-button size="small" type="primary" @click="searchSelectedText">搜索</el-button>
      </div>
    </el-popover>
...
// Popover 引用
const selectionPopover = ref(null)
// 选中的文本内容
...
const handleMouseUp = () => {
  ...
  if (text) {
    ...
    const rect = range.getBoundingClientRect() // 获取选中文本的位置信息
    
    if (referenceEl.value) {
      const { left, top, width } = rect
      referenceEl.value.style.left = `${left + width / 2}px`
      referenceEl.value.style.top = `${top}px`
    }
...
// // 搜索选中的文本（这里只是示例，实际项目中可以跳转到搜索页面）
// const searchSelectedText = () => {
//   if (selectedText.value) {
//     const searchUrl = `https://www.baidu.com/s?wd=${encodeURIComponent(selectedText.value)}`
...
document.addEventListener('click', (e) => {
  const popoverEl = selectionPopover.value?.popperRef?.contentRef?.el; // 获取 Popover 实际 DOM
  const isClickInsidePopover = popoverEl?.contains(e.target);
  
  // 只有点击既不在内容区，也不在 Popover 内时，才关闭
  if (!e.target.closest('.content') && !isClickInsidePopover) {
    popoverVisible.value = false;
  }
});
