<template>
  <div class="flex justify-between fixed top-0 right-0 z-20 h-16 text-base-text items-center px-2" :style="{
    ...
    <div class="flex items-center cursor-pointer flex-1">
      ...
      <button v-if="isMobile" type="button" aria-label="打开菜单" :class="cn(
      ...
      <div v-else-if="!isVertical" class="flex min-w-48 cursor-pointer items-center justify-center"
        @click="router.push({ path: '/' })">
        <Logo />
        <div class="inline-flex font-bold text-2xl ml-2" :class="(effectiveMode === 'head' || effectiveMode === 'combination') &&
          'min-w-fit'
          ">
      ...
      <gva-aside v-if="effectiveMode === 'head' && !isMobile" class="flex-1" />
      <gva-aside v-if="effectiveMode === 'combination' && !isMobile" mode="head" class="flex-1" />
    </div>

    <div class="ml-2 flex items-center gap-3">
      <tools />
      <!-- 0.5px 极细分隔线，区分工具区与用户区 -->
      <div class="h-6 w-px bg-border" role="separator" />
      <!-- hover 触发：鼠标悬停用户胶囊即展开操作菜单；点击胶囊则钉住展开（移出后保持），点击面板外或选中菜单项后收起 -->
      <HoverCardRoot :open="menuOpen" :open-delay="100" :close-delay="120" @update:open="onHoverChange">
        <HoverCardTrigger as-child>
          ...
          <div ref="triggerEl"
            ...
            @pointerdown="pinMenu">
            <CustomPic />
            <span v-show="!isMobile" class="pr-1">{{
        ...
        <HoverCardPortal>
          <HoverCardContent :side-offset="8" side="bottom" align="end"
            ...
            <template v-if="otherAuthorities.length">
              <div v-for="item in otherAuthorities" :key="item.authorityId"
                class="gva-menu-item"
            ...
            <!-- 退出登录（红色语义色） -->
            <div class="gva-menu-item gva-menu-item-danger" @click="handleSelect(userStore.LoginOut)">
