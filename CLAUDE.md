...
一句话记住：**不评价 · 不预测 · 不荐买卖 · 个股只做「客观事实」层（分类归属 + 涨跌幅，**依据可溯源**）·
一期不做个股详情 · 用户自主判断**。

> **分界线是「事实 vs 我方观点」，不是「有没有出现个股」。**
> 证监会公告〔2012〕40 号四项要件的核心动词全是规范性表述——投资分析**意见** / **预测**走势 /
> 选择**建议** / 买卖时机**建议**。"A 公司属该题材成分股""A 公司今日涨 3.2%"是**事实**，不构成荐股；
> "A 是龙头""即将爆发"是**我方观点**，构成。
>
> ⚠️ 但"事实"必须真的是事实：**归属依据若来自我方主观判断，就退化成品种选择**——
> 所以每条「题材↔个股」映射必须存证据（来源+原文摘录+链接+时点），**无依据禁止入库**。
> ⚠️ **免责声明不能豁免**；证券投顾牌照 2016 年起停发。
> ⚠️ 展示涨跌幅另需**行情数据商用授权**（与投顾牌照是两件事），一期用延时行情并明示口径。
>
> v1 的"一期不含个股"口径**已被推翻**，见 [`docs/adr/0003-个股客观事实层可做.md`](docs/adr/0003-个股客观事实层可做.md)。
