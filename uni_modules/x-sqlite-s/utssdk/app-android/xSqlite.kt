...
class xSqliteHelp(private val context: Context, private val config: SQLiteConfigByKt = SQLiteConfigByKt()) {
    ...
    fun createDb(filename: String? = null): SQLiteDatabase? {
        ...
        // 如果 defaultDirectory 配置为空字符串或无效，退回到系统默认数据库目录
        val dir = config.defaultDirectory
        val dbFile = if (!dir.isNullOrEmpty()) {
            File(dir, "$fname.db")
        } else {
            context.getDatabasePath("$fname.db")
        }
        try {
            // 原写法 `!dbFile.parentFile?.exists()!!` 在 parentFile 为 null 时会抛 NPE
            // （例如 defaultDirectory 传了空字符串或无目录前缀的值，File 构造出来的文件没有父目录）
            // 这里改为安全写法：仅在 parent 非空且不存在时创建
            val parent = dbFile.parentFile
            if (parent != null && !parent.exists()) {
                val made = parent.mkdirs()
                if (!made && !parent.exists()) {
                    console.error("xSqlite.createDb: 无法创建数据库父目录 -> ${parent.absolutePath}")
                    return null
                }
            }
            ...
            if (config.encryption && config.password != null) {
                // 注意：Android 原生 SQLite 不支持 PRAGMA key，此 PRAGMA 只在集成 SQLCipher 时生效
                run("PRAGMA key = ?", listOf(config.password!!))
            }
            fileURLPath = dbFile.absolutePath
            return db
        } catch (e: Exception) {
            // 把错误信息打到 JS 层 console，便于排查；同时保留原有 printStackTrace 便于 logcat 定位
            console.error("xSqlite.createDb 失败: ${e.message} (path=${dbFile.absolutePath}, dir=${config.defaultDirectory ?: "<system>"})")
            e.printStackTrace()
            return null
        }
    }
