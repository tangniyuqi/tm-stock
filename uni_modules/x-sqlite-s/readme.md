...
### SQLiteConfig 配置项
**鸿蒙不支持自定义其它目录**为了全平台兼容，不建议设置defaultDirectory，
在安卓和ios中defaultDirectory它可指定为缓存目录，下载目录，用户目录等等沙盒目录。
但鸿蒙它是固定在`应用目录/rdb/自定义目录`，下，因为实际上是被固定在了`应用目录/rdb/`下.
因此差异非常大。故，鸿蒙端defaultDirectory我将不会生效。仅在安卓和ios下生效。就算你写了我也不使用。
鸿蒙一定要离开页面后关闭。否则再次打开时会失败，鸿蒙端他全局允许打开一个db实例，而且是唯一的
